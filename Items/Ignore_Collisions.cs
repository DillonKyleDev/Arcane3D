...
public class Ignore_Collisions : MonoBehaviour
{
    ...
    List<KeyValuePair<string, bool>> ignoreList = new List<KeyValuePair<string, bool>>();
    ...
    private void Start() {
        ...
        //Create new list of only the ignored Tags
        for(int i = 0; i < tagList.Count; i++) {
            if(tagList[i].Value == true) {
                ignoreList.Add(tagList[i]);
            }
        }

        allTags = GameObject.FindObjectsOfType<Tags>();
        //Cycle through all Tag Components
        for(int i = 0; i < allTags.Length; i++) { 
            //For each Tag Component, cycle through each ignorelist Tag
            for(int j = 0; j < ignoreList.Count; j++) {
                
                //If Tag Component has a tag on the ignoreList, ignore it
                if(allTags[i].HasTag(ignoreList[j].Key)) {
                    Physics.IgnoreCollision(gameObject.GetComponent<Collider>(), allTags[i].gameObject.GetComponent<Collider>(), true);
                }
            }
        }
    }
